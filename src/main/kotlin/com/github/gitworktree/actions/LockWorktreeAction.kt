...
class LockWorktreeAction : AnAction() {
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        ProgressManager.getInstance().run(object : Task.Backgroundable(project, GitWorktreeBundle.message("action.lock.worktree")) {
            override fun run(indicator: ProgressIndicator) {
                val cmdResult = GitWorktreeManager.getInstance().lockWorktree(project, repo, worktree.path)
                if (cmdResult.success()) {
                    WorktreePanel.refreshAll(project)
                    notify(project, GitWorktreeBundle.message("notification.lock.success"))
                } else {
                    notify(
                        project,
                        GitWorktreeBundle.message("notification.lock.failed", cmdResult.errorOutputAsJoinedString),
